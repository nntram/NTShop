            ...
            if (Request.Query.Count > 0)
            {
                ...
                if (checkSignature)
                {
                    if (vnp_ResponseCode == "00")
                    {
                        ...
                        var result = "Thanh toán thành công hóa đơn " + orderId + " | Mã giao dịch: " + vnpayTranId;
                        return Ok(result);
                    }
                    else
                    {
                        //Thanh toán không thành công. Mã lỗi: vnp_ResponseCode
                        var result = "Có lỗi xảy ra trong quá trình xử lý hóa đơn " + orderId + " | Mã giao dịch: " + vnpayTranId + " | Mã lỗi: " + vnp_ResponseCode;
                        return StatusCode(500, result);
                    }
                }
                else
                {
                    var result = "Có lỗi xảy ra trong quá trình xử lý";
                    return StatusCode(500, result);
                }
            }
